...
\begin{tikzpicture}[baseline=(current bounding box.base)]
	\begin{feynman}
		...
		\vertex[right=1.25cm of a] (b);
		\vertex[right=of b] (c);
		\vertex[right=1.25cm of c] (d);
		\vertex (u) at ($(b)!0.5!(c)$);
		
		\node[above=0.65cm of u] {$\mathcal{U}^{\textbf{q}}_{\mathfrak{1432}}$};
...
%		
%		%\node (u) at ($(b1)!0.5!(b3)$) {$\mathcal{U}^{\textbf{qkk}'}$};
%		%\draw (u) circle (0.5);
%
%		\node at ($(c1)!0.5!(d2)$) {$F^{\q\k\kp}_{\mathfrak{de2f}}$};
%		
%		\diagram* {
...
%			(b1) -- [photon, edge label'=$\mathcal{U}^{\textbf{q}}_{\mathfrak{a1bc}}$] (b3)
%		};
%	\end{feynman}
%\end{tikzpicture}
\begin{tikzpicture}[baseline=(current bounding box.center)]
...
\begin{tikzpicture}[baseline=(current bounding box.base)]
	\begin{feynman}
		...
		\vertex[right=1.25cm of a4] (a5);
		
		\vertex[above=of a2] (b2);
		\vertex[right=2cm of b2] (b3);
		\vertex[right=of b3] (b4);

		\node at ($(a3)!0.5!(b4)$) {$F^{\q\kp\k}_{\mathfrak{de2f}}$};
		%\node at ($(a2)!0.5!(b3)$) {$\chi^{\q\kp\kp}_{0;\mathfrak{cbed}}$};
		
		\diagram* {
			(a1) -- [fermion, gray, edge label'=$\k;\mathfrak{1}$] (a2) -- [fermion] (a3) -- (a4) -- [fermion, gray, edge label'=$\k;\mathfrak{2}$] (a5),
			(a3) -- (a4) -- (b4) -- (b3) -- (a3),
			(b3) -- [fermion] (b2),
			(a2) -- [photon, edge label=$\mathcal{U}^{\textbf{q}}_{\mathfrak{a1bc}}$] (b2),
			(b2) -- [fermion, bend left=45] (b4),
		};
	\end{feynman}
\end{tikzpicture}
