...
\begin{feynman}
	...
	\node (content1) at ($(b1)!0.5!(c2)$) {$\chi^{*;\q\k\k_1}_{\mathfrak{12ba};\sigma\sigma_1}$};	
	\node (content2) at ($(e1)!0.5!(f2)$) {$\chi^{\q\k_2\kp}_{\mathfrak{dc34};\sigma_2\sigma'}$};
	
	\vertex (u1) at ($(c1)+(0.75,-0.75)$);
	\vertex (u2) at ($(u1)+(1.5,0)$);
	\vertex (u) at ($(u1)!0.5!(u2)$) {$\mathcal{U}^{\textbf{qk}_1\textbf{k}_2}_{\substack{\mathfrak{abcd}\\\sigma_1\sigma_2}}$};
	\draw (u) circle (0.75);
