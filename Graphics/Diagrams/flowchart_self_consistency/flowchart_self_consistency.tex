...
\begin{tikzpicture}	
    ...
    \node (t1) at ($(start.center)+(0,-0.2)$) {\begin{tabular}{c}Retrieve $\varepsilon_{\mathfrak{12}}^{\textbf{k}}$ and $\mathcal{U}^{\textbf{q}}_{\mathfrak{1234}}$ \\ e.g.\ from DFT or cRPA\end{tabular}};
    \node (t2) at ($(dmft.center)+(0,-0.2)$) {\begin{tabular}{c}Extract $\Sigma_{\text{DMFT};\mathfrak{12}}^{\nu}$ \\ and $G_{\dens/\magn;\mathfrak{1234}}^{\ph;\omega\nu\nu'}$ from DMFT\end{tabular}};
    \node (t3) at ($(dyson.center)+(0,-0.2)$) {\begin{tabular}{c}Calculate $G^{\k}_{\mathfrak{12}}$ via \eqref{eq:dyson_eq}\\ and update $\mu$\end{tabular}};
    \node (t4) at ($(bse.center)+(0,-0.2)$) {\begin{tabular}{c}Determine $\chi^{\q}_{\dens/\magn;\mathfrak{1234}}$ via \eqref{eq:chi_phys_asympt} \\ and $\gamma_{\dens/\magn;\mathfrak{1234}}^{\q\nu}$ via \eqref{eq:vrg}\end{tabular}};
