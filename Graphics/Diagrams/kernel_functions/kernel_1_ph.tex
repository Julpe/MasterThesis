...
\begin{tikzpicture}[baseline=(current bounding box.center)]
	\begin{feynman}[small]
		...
		\vertex (u1) at ($(c)!0.5!(d)$) {$\mathcal{U}^{\textbf{q}(-\textbf{k}_1)}_{\substack{\text{ph};\mathfrak{12ab}\\\sigma\sigma_1}}$};
		\vertex (u2) at ($(g)!0.5!(h)$) {$\mathcal{U}^{\textbf{q}\textbf{k}_2}_{\substack{\text{ph};\mathfrak{cd34}\\\sigma_2\sigma'}}$};
		
		\draw (u1) circle (0.75);
		\draw (u2) circle (0.75);
		
		\node at ($(e1)!0.5!(f2)$) {$\chi^{\q\k_1\k_2}_{\substack{\text{ph};\mathfrak{badc}\\\sigma_1\sigma_2}}$};
		
		\diagram* {
			(a1) -- [gray, fermion, edge label'=$\sigma;\mathfrak{1}$] (b1) -- [fermion, gray] (c) -- [fermion, gray] (b2) -- [gray, fermion, edge label'=$\sigma;\mathfrak{2}$] (a2),
