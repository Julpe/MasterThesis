...
\begin{figure}[h!]
	...
	\caption{Differences (third column) between the real and imaginary part of the \textit{local} irreducible vertex in the density channel calculated with \texttt{DGApy} (first column) and the new program (second column).}
...
\begin{figure}[h!]
	...
	\caption{Differences (last column) between the real and imaginary part of the \textit{local} irreducible vertex in the density channel calculated from the one-band dataset (first column) and the fictitious two-band dataset (second column).}
...
\begin{figure}[h!]
	...
	\caption{Similar to \figref{fig:sigma_difference_oneband_twoband_lower_stat} but with a four-times increase in measurement statistics for the one- and two-particle calculations in \texttt{w2dynamics}.}
...
\begin{figure}[h!]
	...
	\caption{Similar to \figref{fig:gamma_difference_dens_oneband_twoband_lower_stat} but with a four-times increase in measurement statistics for the one- and two-particle calculations in \texttt{w2dynamics}.}
...
\begin{figure}[h!]
	...
	\caption{Similar to \figref{fig:gamma_difference_magn_oneband_twoband_lower_stat} but with a four-times increase in measurement statistics for the one- and two-particle calculations in \texttt{w2dynamics}.}
	\label{fig:gamma_difference_magn_oneband_twoband_higher_stat}
\end{figure}

Finally, we also compare the frequency dependence of both the one- and two-band self-energy. For this, we plotted the single-band self-energy compared to the first diagonal ($\mathfrak{11}$) component of the fictitious two-band calculation for both lower (\figref{fig:sigma_difference_oneband_twoband_frequency}\subref{subfig:sigma_difference_oneband_twoband_lower_stat_frequency}) and higher (\figref{fig:sigma_difference_oneband_twoband_frequency}\subref{subfig:sigma_difference_oneband_twoband_higher_stat_frequency}) QMC measurement statistics and $\vv{k}=\vv{0}$.
\begin{figure}[ht!]
	\centering
	...
	\end{subfigure}
	\caption{Frequency dependence of the real and imaginary part of the self-energy for the single-band and the fictitious two-band dataset. \subref{subfig:sigma_difference_oneband_twoband_lower_stat_frequency} shows the comparison for a lower QMC statistics run for the two-band dataset, whereas \subref{subfig:sigma_difference_oneband_twoband_higher_stat_frequency} for a four-times higher one. The improved behaviour for large frequencies is not surprising, as the increase of QMC measurements mitigates some stochastic noise.}
	\label{fig:sigma_difference_oneband_twoband_frequency}
\end{figure}

Taken together, these three tests confirm that our implementation treats disconnected multi-band systems in a consistent and reliable manner. The first two tests established internal consistency within a single dataset by verifying the equivalence of single- and two-band representations derived from the same calculation. The third test then demonstrated that this equivalence is preserved even when comparing truly independent datasets generated by \texttt{w2dynamics}. The results provide strong evidence that our code robustly respects the expected physics of disconnected orbital systems and correctly reproduces the equivalence to independent copies of the single-band model.

\newpage
\subsection{Rotated disconnected two-band model}\label{sec:validation_iii}

For this test, we use the fictitious disconnected two-band model introduced earlier and applied rotations in orbital space. The goal is to verify that the orbitals remain properly separated and are not mixed in unintended ways. To this end, we implemented simple rotation routines in the code, which allow us to rotate the one- and two-particle vertex functions in orbital space. For the one-particle Green's function and self-energy this looks like
\begin{align}
...
\end{align}
To apply a rotation to higher orbital dimensions, one would also have to construct a higher-dimensional rotation matrix with more than one angle parameter. To perform this test, we applied a rotation by an angle $\vartheta$ to all input quantities from DMFT, including the kinetic part of the Hamiltonian and the interaction terms. The $\dga$ algorithm was then used to compute the non-local self-energy, which we subsequently rotated back by $-\vartheta$. We compared the resulting self-energy to a reference calculation performed without any rotations and found them to agree up to floating-point precision, with $\sum_{\k;\mathfrak{12}}|\Sigma_{\mathfrak{12}}^{\k;\vartheta\neq0}-\Sigma_{\mathfrak{12}}^{\k;\vartheta=0}|=0$ for $\vartheta=\frac\pi2,\pi$ and $\sum_{\k;\mathfrak{12}}|\Sigma_{\mathfrak{12}}^{\k;\vartheta\neq0}-\Sigma_{\mathfrak{12}}^{\k;\vartheta=0}|\lesssim 10^{-3}$ for other values of $\vartheta$. We tried this with different numbers of Matsubara frequencies $n_\omega$ and $n_\nu$​ and varying $\vv{k}$-grid sizes --- all yielding indistinguishable results. 

Exemplary results of the self-energy and the irreducible vertex $\Gamma_{\dens/\magn}^{\omega=0;\nu\nu'}$ can be seen in \figref{fig:sigma_difference_rotated_pi_3}, \figref{fig:gamma_difference_rotated_pi_3_dens} and \figref{fig:gamma_difference_rotated_pi_3_magn}. This calculation was done for the fictitious two-band model with a $\vv{k}$-grid of $24\times24\times1$, $n_\omega=60$, $n_{\nu;\text{core}}=40$ and $n_{\nu;\text{asympt}}=0$. Here, we show the $(\mathfrak{11})$ orbital of the self-energy and the $(\mathfrak{1111})$ orbital component of the irreducible vertex; the $(\mathfrak{22})$ and $(\mathfrak{2222})$ components of both quantities look very similar.
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{../../Graphics/Validation/Rotation/sigma_difference_rotated_pi_3}
	\caption{Differences (last column) between the two-band calculation without a rotation ($\vartheta=0$, first column) and a calculation, where the input data was rotated before the $\dga$ calculation and subsequently rotated back by a factor of $\vartheta=\frac\pi3$ (second column) for both the real and imaginary part of the self-energy (first and second row, respectively). }
