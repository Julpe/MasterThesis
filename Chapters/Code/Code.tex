...
\end{align}
compared to our definition of \eqref{eq:sigma_double_counting}. This does not add a significant difference, but it should be noted nevertheless. We carried out the single-band calculation with \eqref{eq:sigma_double_counting_dgapy} and \eqref{eq:sigma_double_counting} and compared them to the results of \texttt{DGApy} separately, both of which can be seen in \figref{fig:sigma_difference_me_dgapy_dgapy_dc} and \figref{fig:sigma_difference_me_dgapy_correct_dc}, respectively.
\begin{figure}[ht!]
	\centering
	\includegraphics[width=\textwidth]{../../Graphics/Validation/DGApy/sigma_difference_me_dgapy_dgapy_dc}
	\caption{Difference between the one-band self-energy of \texttt{DGApy} and the current implementation with the double-counting kernel of \eqref{eq:sigma_double_counting_dgapy} for $k_z=0$ and $\nu=0$. The result of \texttt{DGApy} can be seen in the very left column, the result of the current implementation in the second column. The difference between both, i.e. $\Delta\Sigma=\Sigma_{\texttt{DGApy}}-\Sigma$, can be seen in the last column. The rows indicate the real- and imaginary part of all quantities, respectively. This calculation was done with a $\vv{k}$-grid of $64x64x1$, $n_\omega=60$, $n_{\nu;\text{core}}=40$ and $n_{\nu;\text{asympt}}=20$ without any $\lambda$-correction.}
