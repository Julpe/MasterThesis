...
We discovered a tiny subtlety in the implementation of \texttt{DGApy}. It uses a different double-counting correction, which reads 
\begin{align}\label{eq:sigma_double_counting_dgapy}
	\Sigma^{\k}_{\text{dc};\mathfrak{12}} = \frac{1}{\beta}\sum_{\substack{\mathfrak{abcdef}\\\q\nu'}}\tilde{\mathcal{U}}_{\mathfrak{a1bc}}\chi^{\q\nu'\nu'}_{0;\mathfrak{cbed}}F^{\omega\nu'\nu}_{\magn;\mathfrak{de2f}}G_{\mathfrak{af}}^{\k-\q},
\end{align}
compared to our definition of \eqref{eq:sigma_double_counting}. This does not add a significant difference, but it should be noted nevertheless. We carried out the single-band calculation with \eqref{eq:sigma_double_counting_dgapy} and \eqref{eq:sigma_double_counting} and compared them to the results of \texttt{DGApy} separately, both of which can be seen in \figref{fig:sigma_difference_me_dgapy_dgapy_dc} and \figref{fig:sigma_difference_me_dgapy_correct_dc}, respectively. These calculation were done with a $\vv{k}$-grid of $64x64x1$, $n_\omega=60$, $n_{\nu;\text{core}}=40$ and $n_{\nu;\text{asympt}}=20$ without any $\lambda$-correction.
\begin{figure}[h!]
	\centering
...
For the double-counting kernel of \eqref{eq:sigma_double_counting} we found an agreement of $\mathcal{O}(10^{-3})$, where the largest superconducting eigenvalues differed by $\mathcal{O}(10^{-4})$. We are very certain, that the double-counting kernel of \eqref{eq:sigma_double_counting} should be correct for all multi-/single-orbital cases and the usage of \eqref{eq:sigma_double_counting_dgapy} in \texttt{DGApy} is not clear to us.
...
\begin{figure}[h!]
	...
	\label{fig:sigma_difference_oneband_twoband}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{../../Graphics/Validation/Oneband_vs_Twoband/gamma_difference_dens_oneband_twoband}
	...
	\label{fig:gamma_difference_dens_oneband_twoband}
\end{figure}
\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{../../Graphics/Validation/Oneband_vs_Twoband/gamma_difference_magn_oneband_twoband}
	\caption{Analogous to \figref{fig:gamma_difference_dgapy_dens} but for the magnetic spin combination.}
	\label{fig:gamma_difference_magn_oneband_twoband}
\end{figure}
