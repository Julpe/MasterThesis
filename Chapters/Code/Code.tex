...
\begin{figure}[ht!]
	...
	\caption{Difference between the one-band self-energy of \texttt{DGApy} and the current implementation with the double-counting kernel of \eqref{eq:sigma_double_counting_dgapy} for $k_z=0$ and $\nu=0$. The result of \texttt{DGApy} can be seen in the very left column, the result of the current implementation in the second column. The difference between both, i.e. $\Delta\Sigma=\Sigma_{\texttt{DGApy}}-\Sigma$, can be seen in the last column. The rows indicate the real- and imaginary part of all self-energies, respectively.}
...
The first test consisted of projecting a single orbital out of the disconnected two-band dataset and comparing it to the corresponding orbital of the two-band model. Since both originate from the same underlying data, one expects perfect agreement between the two. Indeed, this is exactly what we observed: both the self-energy and the two-particle vertex functions matched up to numerical precision, with absolute differences vanishing within machine accuracy. This confirmed that our implementation correctly interprets the orbital structure of the disconnected two-band system and does not introduce any discrepancies when restricting attention to a single orbital. Since this test case reproduced the exact numerical results as expected, we will not show any data, as it would be of little to no use.
\newline

As a second test, we carried out the inverse procedure. Starting from the single-band dataset, we artificially embedded it into a disconnected two-band structure and then compared one of the diagonal orbitals from this construction to the original single-band data. Again, this comparison did not involve two independent calculations, but rather served as a stringent internal consistency check on the code's ability to handle single- and multi-band inputs in a uniform manner. As expected, the agreement was again exact up to numerical precision for both the self-energy and the vertex functions, confirming that the code faithfully reproduces the equivalence between the single-band description and its trivial two-band embedding. Since this test case reproduced the exact numerical results as expected, we will not show any data, as it would be of little to no use.
\newline

The third test, in contrast, did involve independent input datasets. Here, we used the CT-HYB solver \texttt{w2dynamics} to generate data for both the single-band and the disconnected two-band systems separately and then compared the results. The two datasets agreed very well overall, demonstrating that the expected equivalence between the two cases is also maintained when the input is generated independently. In this comparison, small deviations were visible, however, this behavior is fully consistent with the stochastic nature of the continuous-time quantum Monte Carlo method employed in \texttt{w2dynamics}. Statistical noise is an unavoidable feature of CT-QMC data, and its presence here is therefore not surprising. Importantly, these fluctuations did not obscure the overall agreement between the single- and two-band results. A comparison between the single- and two-band self-energies and local irreducible vertices can be seen in \figref{}\cite{TODO}.
\newline
