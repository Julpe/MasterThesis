...
\end{align}
In the case where Wannier orbitals on different lattice sites do not overlap (effectively pairing up the operators at sites $\vv{0}$ and $\vv{R}$), the interaction becomes diagonal in real space. As a result, the Coulomb matrix elements depend only on the relative positions of the sites and their Fourier transform depends at most on the momentum transfer $\vv{q}$, while the explicit dependence on $\vv{k}$ and $\vv{k}'$ vanishes,
\begin{subequations}
\begin{align}
