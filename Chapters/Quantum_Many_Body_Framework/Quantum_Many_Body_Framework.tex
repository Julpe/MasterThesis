...
\end{align}
The individual (anti-)symmetric behavior after applying each of the above listed symmetry operations individually uniquely determines the pairing type (singlet/triplet) of the Cooper pairs. This of course limits the solutions of the linearized Eliashberg equations to a small subset, which is important to be aware of when solving them numerically. To be more precise, the corresponding gap function for singlet pairing fulfills $\Delta^{\k}_{\mathfrak{12}}=\Delta^{-\k}_{\mathfrak{21}}$ and the corresponding gap function for triplet pairing fulfills $\Delta^{\k}_{\mathfrak{12}}=-\Delta_{\mathfrak{21}}^{-\k}$.
...
\end{align}
\end{subequations}
where $n$ denotes the iteration number. This in theory is not difficult, the computationally demanding part of solving the linearized Eliashberg equation is the calculation of the irreducible vertex in the particle-particle channel. Following the derivation outlined in the supplemental material of \cite{motoharu why tc is so low} and generalizing the equations to account for multiple orbitals yields for the full vertex in ladder-$\dga$ for $\r\in\{\dens/\magn\}$ spin combinations
\begin{align}
\begin{split}
