...
\begin{align}
	F^{\q\k\kp}_{\mathfrak{1234};\ss'}=\beta^2\left [\left (\chi_{0;\mathfrak{1234}}^{\q\k\k}\right )^{-1}-\sum_{\k\kp;\mathfrak{abcd}}\left (\chi_{0;\mathfrak{12ba}}^{\q\k\k}\right )^{-1} \chi_{\mathfrak{abcd};\ss'}^{\q\k\kp} \left (\chi_{0;\mathfrak{dc34}}^{\q\k\k}\right )^{-1}\right ]^{-1}.
\end{align}
...
\begin{align}
	\Gamma_{\mathfrak{1234};\ss'} = \mathcal{U}^{\vv{qkk}'}_{\mathfrak{1234};\ss'}+\delta\Gamma_{\mathfrak{1234};\ss'}.
\end{align}
It is then straightforward to define an auxiliary susceptibility (denoted by the asterisk $^*$)
\begin{align}
\begin{split}
	\chi_{\mathfrak{1234};\ss'}^{*;\q\k\kp}&=\left [\left (\chi_{0;\mathfrak{1234}}^{\q\k\k}\right )^{-1}+\frac{1}{\beta^2}\delta\Gamma_{\mathfrak{1234};\ss'}\right ]^{-1}\\
	&=\left [\left (\chi_{0;\mathfrak{1234}}^{\q\k\k}\right )^{-1}+\frac{1}{\beta^2}\left (\Gamma_{\mathfrak{1234};\ss'}-\mathcal{U}^{\vv{qkk}'}_{\mathfrak{1234};\ss'}\right )\right ]^{-1},
\end{split}
\end{align}
...
\begin{align}
	\Sigma_{\mathfrak{12};\ss'}^{\k}=\Sigma_{\text{HF};\mathfrak{12}}^{\vv{k}}+\sum_{\substack{\q;\mathfrak{abcd}\\\mathfrak{efgh};\sigma_1}}\mathcal{U}^{\vv{q}}_{\mathfrak{a1bc}}\left [\gamma_{\mathfrak{cbed};\ss'}^{\q\k}\big (\unity_{\mathfrak{de2h}}^{\k\kp}-\mathcal{U}^{\vv{q}}_{\mathfrak{defg};\ss_1}\chi_{\mathfrak{gf2h};\sigma_1\sigma}^{\q}\big)\right ]G_{\mathfrak{ah}}^{\k-\q}
\end{align}
...
From the purely local part of \eqref{eq:sde_with_phbar}, which is contained in the first term in the brackets, one can extract the DMFT self-energy, since the $\ph$-channel contribution with the local $U_{\mathfrak{1234}}$ can be rewritten as
\begin{align}
...
For an alternative formulation using the three-leg vertices already derived in \secref{sec:schwinger_dyson}, we have to rewrite \eqref{eq:sde_with_phbar} with the full ladder vertices $F^{\q\nu\nu'}_{\dens/\magn;\mathfrak{1234}}$,
\begin{align}
...
\begin{align}
	\Sigma^{\k}_{\dga;\mathfrak{12}}&=\Sigma_{\text{H};\mathfrak{12}}^{\vv{k}}-\frac{1}{\beta}\sum_{\substack{\q;\mathfrak{abcd}\\\mathfrak{efgh}}}\Bigg [\!\bigg (\mathcal{U}^{\vv{q}}_{\mathfrak{a1bc}} -\frac12 \tilde{\mathcal{U}}_{\mathfrak{a1bc}} \bigg )\,\gamma_{\dens;\mathfrak{cbed}}^{\q\nu}\!\left (\unity_{\mathfrak{de2h}}^{\nu'\nu}-\mathcal{U}^{\vv{q}}_{\dens;\mathfrak{defg}}\chi^{\q}_{\dens;\mathfrak{gf2h}}\right )\nonumber\\
	&\qquad\qquad\qquad\qquad\qquad-\frac32 \tilde{\mathcal{U}}_{\mathfrak{a1bc}}\gamma_{\magn;\mathfrak{cbed}}^{\q\nu}\!\left (\unity_{\mathfrak{de2h}}^{\nu'\nu}-\mathcal{U}^{\vv{q}}_{\magn;\mathfrak{defg}}\chi^{\q}_{\magn;\mathfrak{gf2h}}\right )\!\!\Bigg]G_{\mathfrak{ah}}^{\k-\q} - \Sigma^{\k}_{\text{dc};\mathfrak{12}}\nonumber\\
\begin{split}
	&=\Sigma_{\text{H};\mathfrak{12}}^{\vv{k}}-\frac{1}{\beta}\sum_{\substack{\q;\mathfrak{abcd}\\\mathfrak{efgh}}}\Bigg [\!\bigg (U_{\mathfrak{a1bc}} + V^{\vv{q}}_{\mathfrak{a1bc}} -\frac12 U_{\mathfrak{acb1}} \bigg )\,\gamma_{\dens;\mathfrak{cbed}}^{\q\nu}\times\\
	&\qquad\qquad\qquad\qquad\qquad\times\Big (\unity_{\mathfrak{de2h}}^{\nu'\nu}-\left (2U_{\mathfrak{defg}}+2V^{\vv{q}}_{\mathfrak{defg}}-U_{\mathfrak{dgfe}}\right )\chi^{\q}_{\dens;\mathfrak{gf2h}}\Big )\\
	&\qquad\qquad\qquad\qquad\qquad-\frac32 U_{\mathfrak{acb1}}\gamma_{\magn;\mathfrak{cbed}}^{\q\nu}\!\left (\unity_{\mathfrak{de2h}}^{\nu'\nu}+U_{\mathfrak{dgfe}}\chi^{\q}_{\magn;\mathfrak{gf2h}}\right )\!\!\Bigg]G_{\mathfrak{ah}}^{\k-\q} - \Sigma^{\k}_{\text{dc};\mathfrak{12}},
\end{split}
\end{align}
where $\Sigma_{\text{H};\mathfrak{12}}^{\vv{k}}$ is now only the Hartree-term, since the Fock-term contribution exactly cancels with a term arising from the $\phbar$ contribution of $\Sigma^{\k}_{\mathfrak{12}}$ in the three-leg formulation.
