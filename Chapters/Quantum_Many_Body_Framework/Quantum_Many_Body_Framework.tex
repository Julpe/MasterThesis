...
After a successful converged ladder-$\dga$ routine, we have the most important non-local one- and two-particle quantities available. Starting from this, we can further study superconductivity with the so-called Eliashberg equations, an extensive diagrammatic framework based upon particle-particle interactions, which lead to the formation of Cooper pairs \cite{eliashberg, other eliashberg paper, putz Dipl A, jan berges Dipl A}. 
Due to the coupling of electrons, we introduce --- apart from the \say{regular} one-particle Green's function --- two \textit{anomalous} Green's functions
\begin{subequations}
\begin{align}
	G_{\mathfrak{12}}^{\vv{k}}&=-\mean{\mathcal{T}\left [\hat{c}_{\k;\mathfrak{1}}\hat{c}^{\dagger}_{\k;\mathfrak{2}}\right ]},\\
	F_{\mathfrak{12}}^{\vv{k}}&=-\mean{\mathcal{T}\left [\hat{c}_{\k;\mathfrak{1}}\hat{c}_{-\k;\mathfrak{2}}\right ]} \qqand\\
	F_{\mathfrak{12}}^{\vv{k};\dagger}&=-\mean{\mathcal{T}\left [\hat{c}^{\dagger}_{-\k;\mathfrak{1}}\hat{c}^{\dagger}_{\k;\mathfrak{2}}\right ]}.
\end{align}
\end{subequations}
Here, $F_{\mathfrak{12}}^{\vv{k}(;\dagger)}$ denote the anomalous Green's functions, which are not to be confused with the full vertex $F^{\q\k\kp}_{\mathfrak{1234}}$. The anomalous Green's functions are rather unusual contractions of either two electrons or two holes, contrary to the regular Green's function. These contractions will vanish in the normal state, however they are of finite value in the superconducting phase.

Starting from the Nambu formalism, where a two-component spinor is introduced, 
Starting from the Bethe-Salpeter equation in the particle-particle channel (\ref{eq:bethe_salpeter_susc_pp}), we can 
