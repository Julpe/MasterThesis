...
\end{align}
\end{subequations}
Note that expressions \eqref{eq:chi_dens} and \eqref{eq:chi_magn} are nothing more than the Fourier transformed versions in \eqref{eq:chi_dens_derivative} and \eqref{eq:chi_magn_derivative}, respectively.
...
Combining in the previous section derived equation \eqref{eq:generalized susceptibility} and \eqref{eq:bethe_salpeter_vertex_channel}, it is possible to write the full vertex as a Dyson-like equation with the generalized susceptibility, reading
\begin{align}
...
\end{align}
where $\text{r}\in\{\text{d},\text{m}\}$ denotes the (m)agnetic or (d)ensity spin combination, which for the irreducible and full vertex, $\Gamma$ and $F$, respectively, is similar to that for $\chi$, see \eqref{eq:chi_magn} and \eqref{eq:chi_dens}.
